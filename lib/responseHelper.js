...
responseHelper._enforceSchemaOnArray = function(items, resourceConfig, callback) {
  if (!(items instanceof Array)) {
    items = [ items ];
  }
  async.map(items, function(item, done) {
    return responseHelper._enforceSchemaOnObject(item, resourceConfig, done);
...
responseHelper._enforceSchemaOnObject = function(item, resourceConfig, callback) {
  debug.validationOutput(JSON.stringify(item));
  Joi.validate(item, resourceConfig.attributes, function (err) {
    if (err) {
      debug.validationError(err.message, JSON.stringify(item));
      return callback(null, null);
    }

    var dataItem = responseHelper._generateDataItem(item, resourceConfig);
    return callback(null, dataItem);
  });
};

responseHelper._generateDataItem = function(item, resourceConfig) {
  var schema = resourceConfig.attributes;
  ...
  item.type = resourceConfig.resource;
  var result = {
    type: item.type,
    id: item.id,
